...
| AI routes B2 + B3 + B4 | Per-workspace provider routes — full operator surface, runtime, and cleanup. **B2 (Migration + observability)** — migration 048 drops `agent_configs.provider` + `agent_configs.model` (one-shot `backend/src/database/migrations/scripts/backfill-routes.js` find-or-creates `provider_routes` rows + encrypts inherited global keys + appends `source: "backfill-routes"` audit rows; `--dry-run` + `--workspace=<id>` + idempotent re-run); `AI_ROUTES_ENABLED` feature flag removed (`resolveProvider` deleted; `resolveRoute` is the single dispatch-resolution path); new `backend/src/aiProvider/capabilityProbe.js` issues a real 1-token network probe via `protocolAdapter.generate` and auto-fires on every `providerRouteRepo.upsert` whose routing-relevant fields changed (auth/model/baseUrl/family/protocol/apiKey); migration 047 + `ai_request_log` table + `backend/src/aiProvider/requestLog.js` with per-workspace storage mode (`none`/`redacted`/`full` via migration 049 `workspaces.aiRequestLogMode` + `aiRequestLogCustomRedactionRules`); built-in PII redactors (email/phone/SSN/card) + workspace-supplied custom regex rules; `dispatcher.js#computeCostForRoute(route, usage)` with 3-tier priority (`route.pricing` operator-set → `MODEL_PRICING` catalog fallback → `null`); `route_name` Prometheus label added to all 4 AI metrics + OTel `ai.route_name` span attribute; vision-heal (`vision.js`) + agent health-check switched from `resolveProvider` to `resolveRoute`. **B3 (Settings UI + operator hardening)** — full `ProviderRoutesTab` in `Settings.jsx` (`.st-pr-*` CSS, no inline styles) with per-row CRUD + inline `ProbeBadge` Test/Re-probe + `WorkspaceSpendCapsPanel` + `ProviderRoutesIO` export/import bar + `AuditLogSubtab` + `AiRequestLogSubtab` + Rotate-key panel with client-side `detectFallbackCycle` preview; backend routes `GET/POST/PATCH/DELETE /settings/provider-routes`, `/probe`, `/rotate-key` (probe-before-persist gate + rollback on failure + `resetRouteBreakers`), `/export` (schema-v1 JSON, secrets stripped), `/import` (3-phase: upsert → fallback-chain rewire → bounded-parallel probe with `IMPORT_PROBE_PARALLELISM=3`; modes `skip`/`overwrite`/`rename`), `/audit` (paginated workspace-scoped filterable by routeId/action/since/before), `/ai-requests` + `/ai-requests/:id/replay` (rejects non-`full`-mode rows with HTTP 400; honours `routeId` override via `protocolAdapter.generate`); migration 050 adds `workspaces.dailySpendCapUsd` + `monthlySpendCapUsd` + `spendAlertThresholdPct`; migration 051 + `ai_response_cache` table + `backend/src/aiProvider/responseCache.js` (sha256 stable-JSON key + expiry double-check + hitCount + savings metric + `INSERT OR REPLACE` + `coalesceInFlight`/`registerInFlight` thundering-herd protection); migration 052 adds `workspaces.spendAlertWebhookUrl` + `spendAlertLastFiredAt` and `backend/src/aiProvider/spendAlert.js` delivers Slack/webhook-compatible payloads via `safeFetch` with 1-hour cooldown; new `backend/src/aiProvider/quotaGuard.js` ships in-memory + Redis-Lua-atomic dual-path token-bucket rate limiter with `checkAndReserve` pre-call + `reportActual` post-call drift correction + `checkSpendCap` summing `ai_request_log.costUsd` over rolling 24h / month-to-date windows; one-shot `backend/src/database/migrations/scripts/compat-to-routes.js` re-encrypts legacy `compat:%` slots under the B1.4 secrets module with `--dry-run` + `--delete-source`; daily 04:30 UTC cache janitor + 05:00 UTC audit retention sweep + 04:45 UTC request-log retention sweep all staggered in `scheduler.js` with graceful-shutdown. **B4 (Hardcoded-string sweep + advanced routing + load tests + docs)** — `_callProviderUnsafe` + `streamText` + `callVisionModel` all dispatch through `protocolAdapter.generate`/`.stream`/`.generateVision` keyed on `route.protocol` (zero hardcoded family strings remain in dispatch hot path); `buildProviderMeta()` derives cloud entries from `CLOUD_DEFAULT_MODELS` (single source of truth, brand colors co-located via `getCloudColor()`); migration 053 drops `agent_configs.fallbackRole` (canonical route-level fallback on `provider_routes.fallbackRouteId` with `ERR_ROUTE_FALLBACK_CYCLE`) + deletes orphaned `api_keys` compat rows; migration 054 + `route_groups` + `route_group_members` tables + `backend/src/aiProvider/routeGroupResolver.js` resolves `cfg.routeId.startsWith("rg-")` to a concrete route at call time using `weighted` (random by weight) / `latency` (lowest-p50 heuristic) / `cost` (cheapest meeting `requiredCaps`) strategies — `resolveRoute` delegates and callers never see the difference; observability parity: all 3 existing AI alerts pivoted from `sum by (provider)` to `sum by (provider, route_name)` so a single bad route surfaces independently + 2 new alerts `AiSpendCapExceeded` + `AiQuotaRejectionRateHigh`; cardinality budget table + cache+quota PromQL queries in `docs/guide/observability.md`; metric reference table updated with `route_name` label + 4 new cache/cost metrics (`app_ai_cache_hits_total` / `app_ai_cache_misses_total` / `app_ai_cache_savings_usd_total` + per-route cost label); `tests/load/dispatch-overhead.js` (p99 < 5ms budget across 10 routes × 7 roles × 1000 calls) + `tests/load/cache-throughput.js` (10k reads + 1k writes < 5s) — manual/nightly, not registered in `run-tests.js`. **`no-code-edits-contract.test.js`** pins the "operator adds a new vendor with zero code edits" E2E — registers a `family: "custom"` route pointing at a mock OpenAI-compat HTTP server and asserts dispatch lands with the operator-supplied apiKey + baseUrl + model. 12 new test files registered in `backend/tests/run-tests.js` (`migration-routeid`, `migration-rollback`, `request-log`, `capability-probe`, `quota-guard`, `response-cache`, `no-code-edits-contract`, `provider-routes-api`, `routes-import-export`, `compat-migration`, `concurrent-dispatch`, plus updates to `agent-config-routes`/`agent-dispatch`/`resolve-route`); 142/142 backend tests passing. New operator docs: `docs/guide/provider-routes.md`, `docs/operations/quotas-and-caching.md`, `docs/operations/request-log.md`, `docs/schema/provider-routes-v1.json` JSON Schema; `docs/changelog.md` updated with breaking-change call-out; `docs/roadmap/ai-provider-bundle.md` bundle-status table flipped. **Breaking:** operators MUST run `backfill-routes.js` BEFORE deploying. `MODEL_PRICING` is no longer read at runtime by adapters (operator-set `route.pricing` is authoritative; catalog is fallback only). `resolveProvider` deleted. | PR #23 |
| AI-005 + B1.x | Multi-agent dispatch + per-workspace provider routes foundation. **AI-005** — `generateText` / `streamText` accept `agentRole` + `workspaceId`; provider resolved via `resolveProvider({ agentRole, workspaceId })` reading `agent_configs` (migration 037, also closes the legacy AI-004 schema gap) with `systemPromptOverride` / `maxTokens` per role. Circuit breakers + sticky fallbacks keyed per-`(provider, agentRole)` via `breakerKey()`. **AI-005c single-agent preservation** — workspaces without `agent_configs` rows collapse `effectiveAgentRole` to `null`, keeping ONE breaker per provider shared across stages so free-tier providers don't pay a 3× wasted-call tax during 429 incidents. AI Prometheus metrics gain an `agent_role` label (bounded cardinality, canonical 7 roles + `"default"`). Pipeline callers thread `agentRole` through `explorer` (crawl), `planner` (journey), `author` (codegen / chat / feedback / API tests), `healer` (vision). Vision-heal (MNT-001) is now end-to-end agent-routed. New `agentHealthCheck.js` ships `validateAgentConfigs` / `assertAgentConfigsHealthy` — pre-flight 1-token probe (with `responseFormat: "text"` to avoid OpenAI JSON-mode false negatives) wired into both `crawler.js` entry points so misconfigured keys fail at minute 0 not minute 9; probes deduped by resolved provider so N roles sharing one key = 1 paid call. `agentConfigRepo.upsert()` rejects `fallbackRole` cycles at save time with `ERR_AGENT_FALLBACK_CYCLE`. Admin-only `POST /api/v1/settings/agent-roles/:role/test` route + `api.testAgentRole(role)` helper + per-row **Test** button in Settings → Agent Roles tab. **B1.x — Per-workspace provider routes foundation.** New `provider_routes` + `provider_route_audit` tables (migrations 035 + 036) carry one row per concrete LLM endpoint per workspace — protocol + baseUrl + model + AES-256-GCM encrypted API key + capabilities + pricing + per-route quotas + caching toggle + self-referential fallback chain. New `backend/src/aiProvider/secrets.js` encrypts at rest via `SENTRI_MASTER_KEY` (fail-fast in production at module load); 5-min in-memory plaintext cache keyed by `routeId` invalidates on rotate / delete. `apiKeyLastFour` is the only secret-shaped field on default SELECT. New `protocolAdapter.js` switches on `route.protocol` and delegates to four `protocols/*.js` modules (`openai`, `anthropic`, `gemini`, `ollama`) consuming resolved route directly (no env reads). Native-streaming-parity fallback: protocols without SDK streaming return `null` from `stream()` and the dispatcher synthesises a single `onToken(text)` call. New `resolveRoute({ agentRole, workspaceId })` returns `{ route, config, effectiveAgentRole }` via priority chain: sticky-fallback > `agent_configs.routeId` > `agent_configs.provider`-column shim (synthesises transient route, fails closed on unknown provider via `ERR_UNKNOWN_PROTOCOL`) > workspace-default env detection. `breakerDiscriminator(route)` strips synthetic `provider:` prefix so single-agent workspaces don't see their breaker state reset on the B2 dispatch flip. `providerRouteRepo.upsert()` rejects `fallbackRouteId` cycles with `ERR_ROUTE_FALLBACK_CYCLE`; every mutation appends a `provider_route_audit` row in the same tx (rotate_key metadata carries `{ lastFour }` ONLY — never plaintext or ciphertext, enforced by construction). Append-only audit log with monotonic-prefix ids (`pra-{16-hex-seq}-{uuid}`) so lexicographic `ORDER BY id DESC` matches insertion order across SQLite + Postgres. **B1.7 — flag-gated dispatch wiring.** `resolveAgentCall()` gains a `process.env.AI_ROUTES_ENABLED === "true"` branch returning `{ provider, config, effectiveAgentRole, route, useRoutes }`. Default off — flag read on every call so test suites can toggle per-case. New tests: `agent-dispatch.test.js` (resolveProvider priority + per-role breaker isolation + AI-005c collapse + fallbackRole cycle + sticky wins), `provider-routes.test.js` (B1.3 — CRUD + cycle + JSON round-trip + rotate_key redaction), `secrets.test.js` (B1.4 — round-trip + tamper-detect + master-key fail-fast via subprocess spawn + rotation isolates ciphertexts; `_reloadMasterKeyForTests` hard-guarded against production), `protocol-adapter.test.js` (B1.5 — protocol switch + fail-closed + streaming-parity fallback), `resolve-route.test.js` (B1.6 — priority chain + disabled-route returns null not silent shim + breakerDiscriminator namespace fix), `chaos-provider.test.js` (B1.8 — HTTP 500/401, NDJSON recovery, ECONNREFUSED retry exhaustion, mid-flight abort with listener-leak detection, opts.timeoutMs, Bearer-header forwarding). All registered in `run-tests.js`. New operator guide at `docs/guide/multi-agent-pipeline.md` + runbook at `docs/operations/secrets-rotation.md` (dual-key re-encrypt script, rolling-restart, rollback, audit-trail requirements). Off by default — workspaces with no `agent_configs` rows behave identically to single-agent mode; `AI_ROUTES_ENABLED=false` is the production setting. Multi-bundle plan for B2 / B3 / B4 lives in [`docs/roadmap/ai-provider-bundle.md`](./docs/roadmap/ai-provider-bundle.md). | PR #22 |
| AI-002 + AI-003 | AI provider modularization + adapter capability hardening with per-call cost tracking. **AI-002**: Decomposed the 1,545-line `backend/src/aiProvider.js` monolith into 7 modules under `backend/src/aiProvider/` — `index.js` (orchestrator, ~280 lines), `registry.js` (mutable state: runtime keys, sticky fallback, circuit breakers, `detectProvider` waterfall), `retry.js` (`withRetry` / `composeSignal` / `isRateLimitError` / `isTransientServerError`), `modelCatalog.js` (pure-data leaf: provider env-map, default models, `VISION_CAPABLE_MODELS`, `CAPABILITIES`, `MODEL_PRICING`, `computeCostUsd`), `providerInfo.js` (`buildProviderMeta` / `getSupportedProviders` / `getProvider*` / `getConfiguredKeys` / `checkOllamaConnection` / `getModelCatalog`), `dispatcher.js` (SSRF-guarded fetch, `normaliseMessages`, `adapterFor`, `buildAdapterOpts`, `recordAiTokens`, instrumented `callProvider`), `vision.js` (MNT-001 `resolveVisionModel` / `hasVisionProvider` / `callVisionModel`) + 4 adapters under `adapters/` (`anthropic.js`, `openai.js` covering OpenAI/OpenRouter/compat shared SDK, `google.js`, `ollama.js`). Legacy `backend/src/aiProvider.js` is now a 1-line `export * from "./aiProvider/index.js"` shim — every existing import site keeps working without changes. Adapter contract locked at `generate({messages, maxTokens, signal, useJson, model, apiKey, baseUrl}) → {text, usage}` + `stream(same, onToken) → {text, usage} \| null` + `generateVision(opts) → {text, usage} \| null` (null sentinel signals the caller to fall back to non-streaming `generate()` — Google/Ollama emit null for both). Zero behaviour change verified: detection priority preserved (sticky-fallback → quick-switch override → `AI_PROVIDER` env → cloud auto-detect → compat slots → Ollama), SSRF guard on compat baseUrls preserved (`createSsrfGuardedFetch` with `ALLOW_PRIVATE_URLS` escape hatch), `DEMO_GOOGLE_API_KEY` fallback preserved, FEA-003 circuit breaker semantics preserved (1-failure threshold, 5-min cooldown, same-tier fallback only), INF-007 token telemetry preserved bit-for-bit, sticky-fallback TTL preserved. Dependency graph is strictly acyclic with `modelCatalog.js` as a pure-data leaf (no sibling imports) — `registry → modelCatalog`, `providerInfo → modelCatalog + registry`, `dispatcher → modelCatalog + registry + providerInfo`, `vision → providerInfo + dispatcher + modelCatalog`, `index → all of the above`. Pure-arithmetic tests can import `modelCatalog.js` without pulling SDK / DB / metrics dependencies. **AI-003**: `MODEL_PRICING` table extends `modelCatalog.js` with per-(provider, model) `inputPer1k` / `outputPer1k` / `asOf` USD rates for Anthropic (Claude Sonnet 4, Claude 3.5 Sonnet, Claude 3 Opus), OpenAI (gpt-4o / gpt-4o-mini / gpt-4-turbo / gpt-4-vision-preview), Google (Gemini 2.5 Flash / 1.5 Flash / 1.5 Pro), `openrouter/auto` (explicit `null/null` — variable underlying model), and canonical Ollama models (`mistral:7b` / `llama3:8b` / `llama3.1:8b` / `llama3.2:3b` / `qwen2.5:7b` / `phi3:mini` / `gemma2:9b` at `0/0`). New `computeCostUsd(model, usage)` shared helper returns `null` for catalog misses (no fake zeros — `null` is the dashboard's "no data" signal, distinct from a known-free `0`) and `0` for catalog-known free Ollama models. `CAPABILITIES` table extended with `supportsVision` / `supportsJsonMode` / `supportsStreaming` / `contextWindow` / `maxOutputTokens` per provider (`supportsStreaming: false` for Google + Ollama because their adapters' `stream()` returns the `null` sentinel — pinned by `aiProvider-adapter-contract.test.js`); `capabilitiesFor(provider)` returns conservative all-`false` / all-`null` defaults for unknown providers (so a typo can't silently promise vision support) and OpenAI-shape defaults for compat slots (`supportsVision: false` until per-slot metadata lands in a future PR). Every adapter attaches `usage.costUsd` via a `withCost(model, usage)` helper backed by `computeCostUsd`. `app_ai_cost_usd_total{provider, operation}` Prometheus counter generalised — now bumped on every generation + vision-heal call from the catalog-derived `costUsd`, not just MNT-001 vision-heal. MNT-001 `$5/M input + $15/M output` midpoint preserved as a vision-heal fallback when the resolved model isn't in the catalog so `visionHealMaxCostUsdPerMonth` budget circuit-breaker always has *some* signal. Bugfix landed alongside: replaced `Number(usage?.costUsd)` with an explicit `null` check at the vision-heal cost-decision site — the old code coerced catalog-miss `costUsd: null` into `0` and silently disabled the monthly USD cap for any vision model not in `MODEL_PRICING`; regression test `vision-heal: catalog-miss costUsd: null must NOT short-circuit to $0` pins the fix. New `backend/tests/ai-provider-cost-tracking.test.js` pins per-provider arithmetic (Anthropic $3/M+$15/M, OpenAI gpt-4o-mini $0.15/M+$0.6/M, Google Gemini 2.5 Flash $0.075/M+$0.3/M), catalog-miss null behaviour, `openrouter/auto` null pricing, MNT-001 fallback formula, capability-schema completeness, Gemini/Ollama `supportsStreaming: false`, compat-slot inheritance, and the `Number(null)` bug regression. New `backend/tests/aiProvider-adapter-contract.test.js` pins the per-adapter return shape (`generate / stream / generateVision`). Operator guide at `docs/guide/ai-cost-tracking.md` documents the one-file pricing-refresh workflow with a verbatim PR-template block, `asOf` staleness contract, catalog-miss vs known-free distinction, recommended Grafana PromQL queries, capability-flag table, and AI-007 follow-on scope. `docs/changelog.md` updated under `### Added` (AI-003) and `### Changed` (AI-002). JSDoc CI fix: `@param`/`@returns` annotations in `modelCatalog.js` no longer use TypeScript-style `?:` optional-property syntax (JSDoc's type parser rejects them — replaced with `Object` plus prose). Architecture is now ready for AI-004 (agent role config schema) and AI-005 (multi-agent dispatch) without touching any provider state, dispatch, or adapter code — the planner can read `getModelCatalog()` to choose a model and per-role circuit breakers can extend `registry.js#breakerKey(provider, role)` in isolation. | PR #20 |
